...
| 48 | **Xác nhận lượt quét OWASP sau khi nâng cấp thật sự XANH** | WS-10/T10.6 | Lượt `security-scan` kế tiếp | 🟡 **Vòng 2 đo được 18/8**: **56 → 6 CVE ≥ 7**, suppression Kotlin ăn đúng, **bộ nhớ đệm NVD lưu được** (bước 7 xanh — bản vá `if: always()` hoạt động). 6 mã còn lại đều ở `tomcat-embed-core 10.1.55` do BOM Boot ghim; đè `<tomcat.version>10.1.57</tomcat.version>` vá 5 mã (cả 5 có `versionEndExcluding: 10.1.57`), mã thứ 6 vào suppression. **Chờ vòng 3 xác nhận về 0** |
