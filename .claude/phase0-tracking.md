...
| 46 | ⚠⚠ **Quét OWASP lượt thật đầu tiên ĐỎ — và không có đường nâng cấp.** Cổng `failBuildOnCVSS=7` bắt được lỗ hổng ở `spring-boot 3.5.3` (CVE-2026-40971 **9.1**, CVE-2026-22733 8.1, 40972/40975 7.5), `kotlin-stdlib 1.9.25` (CVE-2026-53914 **9.8**, tới qua `minio 8.5.17 → okhttp 4.12.0`), `tomcat-embed-core 10.1.42` (CVE-2025-52520 7.5), `spring-core`/`spring-web` 6.2.8, `spring-security-crypto` 6.5.1, `jackson-databind` 2.19.1. **Kiểm chứng trên Maven Central: 3.5.3 (20/6/2025) là bản Spring Boot MỚI NHẤT** — không có bản vá để lên. Cần quyết định: lập `dependency-check-suppressions.xml` **có ngày hết hạn** cho mục đã thẩm định (không áp dụng / chưa có bản vá) hay chấp nhận job đỏ thường trực. ⛔ **Đỏ thường trực là kết cục tệ nhất** — đúng loại "cảnh báo không ai đọc nữa" mà `conventions.md` §1.5 nói tới | WS-10/T10.6 | **Cần quyết định** | ⬜ Chờ |
