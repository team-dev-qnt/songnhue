...
| 48 | **Xác nhận lượt quét OWASP sau khi nâng cấp thật sự XANH** | WS-10/T10.6 | Lượt `security-scan` kế tiếp | 🟡 **Ba vòng đo 18/8: 66 → 9 → 3 CVE ≥ 7** *(số cũ "56 → 6 → 0" là do tôi đếm sai — xem dưới)*. Vòng 2 xác nhận **bộ nhớ đệm NVD lưu được** (bản vá `if: always()` hoạt động); vòng 3 xác nhận **tomcat 10.1.57 + 2 suppression ăn đúng** — Tomcat biến mất hoàn toàn. ⚠⚠ **Đếm sai vì đọc nhầm thang điểm**: DC in điểm **CVSS v4** trong thông báo lỗi nhưng chặn theo **điểm cao nhất mọi thang**, nên `CVE-2026-34479(6.9)` xuất hiện dưới tiêu đề "≥ 7.0" — nó có **v3 = 7.5**. Script của tôi để v4 ghi đè v3 → tưởng đã sạch trong khi còn 3 mã. Luật đọc báo cáo ghi ở `conventions.md` §4.5 mục 4. Vòng 4 xử nốt: `log4j2.version` 2.24.3→**2.25.5** · `swagger-ui` 5.32.2→**5.32.13** (DOMPurify 3.3.2→3.4.13). **Chờ vòng 4 xác nhận về 0** |
