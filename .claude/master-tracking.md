...
- [ ] T11.3-b: ⚠⚠ **Cluster staging ĐANG CHẠY vẫn sai collation** — nó dựng 25/8, trước khi có bản vá T11.3-a | Note: **vá tệp compose KHÔNG chữa được**, đo 26/8: sửa `compose.prod.yml` rồi `up -d --force-recreate` vẫn ra collation cũ, vì `initdb` chỉ chạy lúc TẠO cluster. Phải dump → `docker volume rm songnhue_postgres-data` → dựng lại → restore (`deploy-guideline.md` §2.7, quy trình đã chạy thật 26/8 trên một cluster sai: dữ liệu giữ nguyên, thứ tự đổi từ `Anh<Đăng<Dung<Em` sang `Anh<Dung<Đăng<Em`). ⭐ Làm luôn lượt này thì đóng kèm được **DOD0.21** (quay lui) và **T7.13** (khôi phục thật + RTO). Từ 26/8 smoke test câu [1/4] sẽ ĐỎ mọi lượt deploy staging cho tới khi làm — cố ý. **CHUẨN BỊ ĐÃ XONG 26/8, chỉ còn bước phá huỷ**: `deploy/` đã rsync lên VPS-2 (compose + `kiem-collation.sh` có mặt, `chmod +x` xong) · script chạy trên cluster thật báo đúng `c | collate=en_US.utf8` / `Anh<Đăng<Dung<Em` · **hai bản chụp đã có, checksum đã đối chiếu**: `predeploy-songnhue-20260826-221542.dump` (312K, `pg_restore -l` đọc được 660 mục lục · 58 TABLE DATA · 6 EXTENSION) và `T11-3b-dumpall.sql` (370K, gồm cả role) · vân tay trước khi đụng: **61 bảng, 27 bảng có dữ liệu** (`articles=9` `settings=92` `role_permissions=334` `permissions=88` `flyway_schema_history=33` `spatial_ref_sys=8500`) · volume dự phòng `songnhue_postgres-data-T11-3b-backup` đã tạo để bước xoá **quay lui được** · `nginx -t` trên container nháp: exit 0. ⛔ Còn lại: dừng writer → chép volume sang bản dự phòng → `docker volume rm songnhue_postgres-data` → `up -d postgres` → `pg_restore` → đối chiếu vân tay → bật lại. **Cần người cho phép thao tác phá huỷ trên máy chủ đang chạy**
