...
- [x] T22.23: Nợ #46 — context đóng gói image ở `dev` | Note: **áp 26/8**, `dev` 2 → **7** context. Thêm `Đóng gói image` · hai job `Đóng gói image frontend (...)` · và `Soi phụ thuộc PR thêm vào` (luôn báo cáo ở PR nhờ `if: github.event_name == 'pull_request'`). ⚠ `Gắn tag SHA cho image không đổi` CỐ Ý không đưa vào — nó chỉ chạy ở `push`, đưa vào là mọi PR đứng vĩnh viễn ở *Expected* (§2.1)
