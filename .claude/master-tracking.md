...
- [x] T11.25: `minio-init` ĐO tài khoản dịch vụ thay vì khai báo nó | Date: 26/08/2026 | Note: Hai `|| true` sau `mc admin user add` và `policy attach` CÓ lý do chính đáng (add hỏng khi tài khoản đã có, attach hỏng khi policy đã gắn — chạy lại lượt deploy là chuyện thường), nên cách chữa KHÔNG phải bỏ chúng đi mà là **đo trạng thái cuối bằng chính cặp khoá ứng dụng sẽ dùng**: `mc alias set svc` rồi ghi→đọc→xoá một đối tượng thử. Mã thoát của `mc admin` chỉ nói lệnh chạy xong, không nói quyền có hiệu lực (luật 9). `add` nay tách nhánh bằng `user info` nên lỗi thật dừng deploy ngay; `attach` giữ `|| true` và điều đó AN TOÀN vì probe bắt được hậu quả. Đo bằng **MinIO thật**, 4 kịch bản: lượt đầu ✓ · chạy lại ✓ (idempotent) · **secret sai độ dài → bản cũ thoát 0 in `✓ sẵn sàng`, bản mới thoát 1 không in ✓** · **policy gắn hụt (tên policy sai, lỗi bị nuốt) → thoát 1 kèm `Insufficient permissions ... /.songnhue-init-probe`** — kịch bản 4 là bài kiểm cho CHÍNH cơ chế mới (luật 7). ⚠ Lượt đo đầu báo 'bản mới cũng thoát 0' — hỏng ở DỤNG CỤ ĐO: hàm bọc `docker run` qua ống `| tail -4`, zsh không có `PIPESTATUS` nên đọc mã thoát của `tail`. Một phép đo hỏng trông y hệt một bản vá hỏng. +1 bài `SeedGateTest`. §10.55
- [x] T11.26: Bỏ công tắc thứ hai của bộ seed — lệch thành không biểu diễn được | Date: 26/08/2026 | Note: Biến thứ hai không mang thông tin gì (bind mount đã ghim `/seed-media`); nó chỉ có MỘT khả năng là lệch với biến thứ nhất, và lệch là hỏng câm (có hàng không byte → `GET /files/<id>` trả 404; có byte không hàng → tệp nằm không ai đọc). `SeedGateTest.haiVeKhongDuocLech` canh đúng chỗ đó nhưng soi **tệp mẫu trong repo**, không soi `/opt/songnhue/.env` đang chạy — canh được bản mẫu, không canh được thứ quyết định hành vi (luật 12). Nay `minio-init` đọc thẳng `SEED_LOCATION`, `/seed-media` cố định. Bài canh cặp thay bằng bài canh **bản vá không bị hoàn tác**: quét TOÀN BỘ `deploy/`, tên biến cũ không được xuất hiện lại — vì nếu ai tách lại hai vế thì họ sẽ không có bài canh cặp nào đỡ, bài ấy đã gỡ cùng lúc. 📌 Tên biến cũ trong bài kiểm ghép từ hai mảnh để mã nguồn bộ canh không khớp phép canh của chính nó — cái bẫy chú thích đã dính 3 lần trong đợt này. Sửa: `compose.prod.yml` · 2 tệp env mẫu · `deploy/seed/README.md` · 2 dòng chẩn đoán trong `deploy.yml`. ⛔ KHÔNG cần thao tác tay trên máy chủ: biến bị bỏ đơn giản là không còn ai đọc. Gói `deploy` 48/48 xanh. §10.55
...
- [ ] T11.30: **Phase sau** — endpoint công khai cho thư viện ảnh công trình (`HomeMediaGallery`) và cho danh sách đơn vị trực thuộc (`org_units`). Hiện chưa có đường nào để cổng lấy hai bộ dữ liệu này, nên component nhận `photos = []` và hiển thị `EmptyBlock`. Không chặn WS-11; ghi lại để lượt dựng MOD-02/MOD-04 không phải khám phá lại. Mở ra từ §10.54, QuanTran chốt 26/08/2026
- [x] T11.29: 4 ô `EmptyBlock` thường trực trên trang chủ — QuanTran chốt **giữ nguyên** | Date: 26/08/2026 | Note: Thuỷ văn (MOD-03) · văn bản điều hành (CN-01.7) · thư viện ảnh · đơn vị trực thuộc (`org_units`). Giữ ô rỗng có lý do thay vì ẩn khối: ẩn đi thì không ai biết chỗ ấy đang thiếu gì, mà đó chính là cách 19 bài viết bịa tồn tại 5 tuần không ai thấy (§10.54). Thư viện ảnh cần endpoint công khai — xem T11.30.
