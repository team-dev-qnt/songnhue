...
- [x] T11.17: Vá lượt nạp nội dung staging hỏng ở lần chạy thử đầu — và 3 script sao lưu cùng lỗi | Date: 25/08/2026 | Note: (1) Bản tóm tắt `if: always()` in Y HỆT nhau ở lượt xanh và lượt đỏ nên người bấm đọc lượt exit 127 thành 'đã ổn' — nay in thẳng ĐẠT/HỎNG từ `job.status`, và nói rõ chạy thử KHÔNG in bảng đối chiếu. (2) Nguyên nhân: `seed.sh` `source` tệp .env, mà `ROBOTS_TAG=noindex, nofollow` hợp lệ với Compose lại khiến shell CHẠY `nofollow` như lệnh. Chỉ staging có giá trị nhiều từ — đo thật: `source prod.env.example` chạy lọt, `staging.env.example` chết ở dòng 137 (luật 24). (3) ⚠⚠ Bài kiểm mới đỏ ngay và chỉ ra backup.sh/restore.sh/pre-deploy-dump.sh cùng dòng ấy — TOÀN BỘ đường sao lưu, lưới an toàn duy nhất vì không có replica/PITR. CHƯA kiểm cron sao lưu đã cài trên staging chưa; chỉ khẳng định được là nếu chạy với tệp ấy thì chết đúng kiểu này. Vá bằng một bộ đọc duy nhất `deploy/lib/read-env.sh` theo luật compose-go, `shlex.quote` nên `eval` an toàn; chạy thật với 5 hình dạng ác ý gồm mật khẩu chứa `#`, `$`, nháy và `; touch` — không lệnh nào bị chạy. `EnvFileNotShellTest` 2 bài, kiểm chứng ngược 2 đỏ/khôi phục xanh. §10.46
