...
- [ ] T11.3-b: ⚠⚠ **Cluster staging ĐANG CHẠY vẫn sai collation** — nó dựng 25/8, trước khi có bản vá T11.3-a | Note: **vá tệp compose KHÔNG chữa được**, đo 26/8: sửa `compose.prod.yml` rồi `up -d --force-recreate` vẫn ra collation cũ, vì `initdb` chỉ chạy lúc TẠO cluster. Phải dump → `docker volume rm songnhue_postgres-data` → dựng lại → restore (`deploy-guideline.md` §2.7, quy trình đã chạy thật 26/8 trên một cluster sai: dữ liệu giữ nguyên, thứ tự đổi từ `Anh<Đăng<Dung<Em` sang `Anh<Dung<Đăng<Em`). ⭐ Làm luôn lượt này thì đóng kèm được **DOD0.21** (quay lui) và **T7.13** (khôi phục thật + RTO). Từ 26/8 smoke test câu [1/4] sẽ ĐỎ mọi lượt deploy staging cho tới khi làm — cố ý
- [ ] T11.6-a: Nhận nợ WS-4 — chặn `/swagger-ui/` và `/v3/api-docs/` ở nginx production
- [~] T11.7: Secrets và biến | Note: rà GitHub 26/8 — ✅ repo secret `NVD_API_KEY` (đúng cấp repo, không phải environment) · ✅ environment `staging` đủ 4 secret · ⛔ **environment `production` KHÔNG có secret nào** (`PROD_HOST`/`PROD_USER`/`PROD_SSH_KEY`/`PROD_BASE_URL`). Thiếu thì CD Production **cảnh báo rồi bỏ qua bước triển khai** chứ không đỏ — tức một lượt deploy production sẽ trông như đã chạy
- [ ] T11.7-a: Nhận nợ WS-22 — đặt **biến kho** `PUBLIC_SITE_URL` rồi dựng lại image `public-web` | Note: **xác nhận 26/8** — `gh api repos/…/actions/variables` trả về RỖNG, tức image đang chạy có `NEXT_PUBLIC_SITE_URL=''` và `sitemap.xml`/canonical của staging đang trỏ `http://localhost:3000` (§10.38)
...
- [ ] T11.40: **Secret scanning + push protection + Dependabot security updates đều `disabled`** trên repo | Note: lộ ra khi rà `gh api repos/…` 26/8. Repo có khoá tài khoản dịch vụ Google, khoá SSH triển khai, khoá AES/JWT — push protection là lớp chặn RẺ NHẤT cho loại tai nạn ấy, và `verify-no-keys.sh` chỉ soi bản dump chứ không soi lượt push. Bấm ở Settings → Code security
- [ ] T11.39: Nợ #27 — 2 mục bảo vệ nhánh | Note: **xác nhận qua `gh api` 26/8**: `staging` và `production` đều còn `strict=true` (phải tắt ở hai chặng đề bạt — chúng không build lại, bắt cập nhật nhánh chỉ tạo vòng merge vô ích); `dev` thiếu context `Vùng nào thay đổi`. Chi tiết: `docs/branch-protection.md` §6.2
...
- [ ] T22.23: Nợ #46 — thêm 3 context đóng gói image vào `required_status_checks` của `dev` | Note: **xác nhận 26/8** — `dev` mới có đúng 2 context (`Backend — build, lint, test`, `Frontend — lint`). Ba job đóng gói image ĐÃ chạy ở PR và xanh (run #32882796669) nhưng chưa bắt buộc, nên chúng chỉ *hiện* lỗi chứ không *chặn* merge (§10.38). Nguyên văn cũ tiếp — script + tên context đầy đủ ở `docs/branch-protection.md` §4.1. Chưa bật thì hai job ấy chỉ *hiện* lỗi ở PR chứ không *chặn* merge
