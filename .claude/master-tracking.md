...
- [ ] T11.3-a: ⚠⚠ **`POSTGRES_INITDB_ARGS` VẮNG HẲN ở `compose.prod.yml`** | Note: **rà 26/8 — nặng hơn cách nó từng được ghi.** `compose.infra.yml` (máy dev) có `--encoding=UTF8 --locale-provider=icu --icu-locale=vi-VN --locale=C.UTF-8`; service `postgres` của `compose.prod.yml` **không có dòng nào**, và `deploy/postgres/init/` cũng không đặt locale. ⛔ `initdb` chỉ chạy MỘT LẦN lúc tạo cluster — đổi sau là phải dựng lại cluster và nạp lại dữ liệu. Hệ quả: `ORDER BY` tiếng Việt xếp sai ở mọi danh sách. **CSDL staging đang chạy đã sai rồi.** Phải chốt trước khi dựng VPS-1, và cần một bài canh hai tệp không được lệch (luật 14)
