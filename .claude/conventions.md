...
- ⚠⚠ **Nhánh sống lâu (`staging`, `production`) chỉ nhận MERGE COMMIT — cấm Squash, cấm Rebase.** Luật ngay trên nói về nhánh *nguồn* đã chết; ở đây nặng hơn một bậc vì nhánh *đích* **không bao giờ bị xoá**. PR #72 (`dev → staging`) gộp bằng Squash sinh `b4a0ac0` **một cha**: nội dung khớp `dev`, quan hệ cha thì không. Gốc chung kẹt lại ở `bbe0b50`, và lượt đề bạt kế tiếp (#76) mở ra với **13 tệp xung đột giả** — hai bên nội dung giống hệt nhau.
  - ⛔ **Cái đắt nhất không phải xung đột.** PR đụng độ thì GitHub **không dựng được `refs/pull/N/merge`**, nên mọi workflow `pull_request` của nó **không bao giờ được lên lịch**: `Promotion guard` — cổng bắt buộc DUY NHẤT của `staging` — treo vĩnh viễn ở *"Expected — waiting for status to be reported"*. Không một dòng đỏ nào để đọc. Cùng họ với luật `skipped` được tính là ĐẠT: **thứ nguy hiểm là sự VẮNG MẶT, không phải màu đỏ.**
  - Bất biến phải giữ: `git rev-list --count --no-merges <nguồn>..<đích>` **= 0**. Merge commit ở nhánh đích là bình thường (đó là các lượt đề bạt trước); commit **một cha** mới là vân tay của squash — đếm bằng `git rev-list --parents -n1 <sha> | wc -w` trừ 1.
  - Canh bằng `.github/scripts/kiem-goc-chung.sh <nguồn> <đích>` gọi trong `promotion-guard.yml`; `PromotionAncestryTest` (6 bài) dựng kho git thật trong `@TempDir` và **tái hiện đúng lượt squash ấy** để chứng minh bộ canh kêu (luật 1).
  - Chữa: `git merge -s ours origin/<đích>` trên nhánh cắt từ `<nguồn>` — ghi lại quan hệ cha mà **không đổi một byte nội dung**; khẳng định `git rev-parse HEAD^{tree}` trước = sau rồi mới phụ thuộc vào nó.
- ⚠ **Một bản vá làm hệ thống SỐNG SÓT qua lỗi cũng là một bản vá làm TẮT chuông báo lỗi ấy.** §10.42 dạy CD giải đỉnh `dev` theo cây tệp nên lượt deploy sống sót qua squash — đúng và cần. Cái giá không ai ghi lại: nó đổi một lần **dừng hẳn** lấy một dòng `::warning::` trên một lượt chạy **màu xanh**. Chuông đã kêu đúng nguyên nhân lúc **31/8 23:54:54** (dòng 194, lượt `33452639951`) và trôi qua. Hạ mức một cảnh báo thì phải nêu ra **ai còn đọc nó** — `docs/cicd.md` §9 khi ấy vẫn khẳng định *"làm sai ở vế thứ hai thì hỏng to tiếng"*, một câu đã hết đúng từ ngày §10.42 vào kho.
- ⚠⚠ **`conclusion: null` của một check-run nghĩa là ĐANG CHẠY, không phải HỎNG.** Cổng đề bạt vừa dựng xong đỏ giả ngay lượt chạy ĐẦU TIÊN với câu `Backend — build, lint, test … kết thúc với 'null'` — một cuộc đua **19 giây** giữa lượt gộp và lượt CI của nó. Máy trạng thái đọc API phải để "chưa kết thúc" thành một nhánh RIÊNG (luật 9). Ba nhánh, và **hỏng thắng chưa-xong**: `.github/scripts/phan-loai-check-chang-truoc.sh` trả `0` đạt / `2` còn chạy / `1` hỏng; workflow đợi tối đa 10 phút rồi **đỏ**, không bao giờ đi tiếp khi hết giờ.
  - Tách khỏi `run:` thành script để nhánh `null` **kiểm được bằng dữ liệu giả** — nằm trong `run:` thì muốn tái hiện phải thắng một cuộc đua (`PromotionCheckStateTest`, 6 bài).
- ⚠⚠ **Nối ống là mất mã thoát.** `./mvnw … | tail -40` in `MÃ THOÁT THẬT = 0` trong khi Maven đã **thất bại** — `$?` là mã của `tail`. Mọi lượt kiểm chứng phải ghi ra tệp rồi lấy `$?` ngay dòng sau, hoặc đọc `${PIPESTATUS[0]}` **dưới `bash`**. ⚠ Đo 1/9 ngay khi viết luật này: shell mặc định của máy dev là **zsh**, ở đó `${PIPESTATUS[0]}` trả **chuỗi rỗng** (zsh dùng `$pipestatus[1]`, đánh số từ 1) — nên câu lệnh phòng thân lại in ra một dòng trống trông như *chưa chạy*. Cùng họ luật 20: cách kiểm cũng phụ thuộc shell.
...
>
> ⚠⚠ **Và tên trường THẬT của báo cáo ODC 12.1.3** (đo 6/9 trên lượt `33951186299`): `cvssv2.score` · `cvssv3.baseScore` · `cvssv4.baseScore` — **KHÔNG phải** `cvssvX.cvssData.baseScore` như JSON của API NVD. Bản nháp đầu của `van-tay-cve.sh` viết theo hình dạng API cho `ge7=0` trên một báo cáo có **6 mã ≥ 7** — xanh giả hoàn hảo, và cùng một lỗi làm một script phân tích trong phiên 6/9 in ra RỖNG mà không ai để ý. Bộ lọc nay đọc cả hai hình dạng; `VanTayCveTest` V9 neo một trích đoạn báo cáo thật để bộ lọc kiểu API NVD phải đỏ ở đó (luật 25). Script chuông đọc **văn bản thuần** do `van-tay-cve.sh` sinh (`ge7=`/`tong=`/`suppress=` + `mã<TAB>điểm<TAB>>=7|<7<TAB>jar`), không đọc JSON (§10.76).
...
7. Cổng bắt buộc của nhánh đích **đã thật sự CHẠY** — không nằm ở trạng thái *Expected*. Một cổng KHÔNG CHẠY không đọc như một cổng ĐỎ, và GitHub không phân biệt hộ: `mergeable=CONFLICTING` là dấu hiệu đầu tiên phải soi, vì nó ngăn chính workflow `pull_request` của PR ấy được lên lịch.
8. PR đề bạt (`dev → staging`, `staging → production`) gộp bằng **Create a merge commit**. Squash làm gãy gốc chung của một nhánh không bao giờ bị xoá (§1.5).
