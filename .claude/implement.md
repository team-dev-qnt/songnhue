...
| **Phase 3** — C3 (GIS/dashboard/báo cáo) + D (HRM) | | 🟨 **ĐANG CHẠY — D (HRM) 5/9 chức năng** | ⛔⛔ **C3 ⛔ không phải cửa vào**, nhưng lý do đã ĐO LẠI 10/09 (T52.8 — câu cũ ở dòng này sai): `constructions` **11 hàng**, `station_constructions` **15**, tuyến sông **7** — thứ **thật sự** rỗng là **TOẠ ĐỘ** (`stations` 19/19 và `constructions` 11/11 đều NULL) ⇒ mọi lớp GIS rỗng. G8 ⛔ không chặn *viết* C3, nó chặn **nghiệm thu** C3. ⭐ **D (HRM) đã dựng**: ✅ CN-04.2 hồ sơ + chức vụ + trường 🔒 (WS-51) · ✅ CN-04.3/04.4/04.5 lý lịch + timeline + tài liệu (WS-53) · ✅ **T51.8** liên kết `users.employee_id` + vế hai CN-04.7 (WS-54) · ✅ **CN-04.6 danh bạ** (WS-55). ⬜ Còn: CN-04.1 sơ đồ tổ chức · CN-04.8 báo cáo · CN-04.9 nghỉ phép (nay **đã mở khoá** — tiền đề T51.8 đã trả). **G6** chặn **đúng một** báo cáo `BCNS-07`; **G6-a** chặn **dữ liệu**, ⛔ không chặn mã — bảng để RỖNG là đúng. ⭐ Phần C3 làm được ngay: **wall mode** (thiết bị chốt B8) + **báo cáo MOD-02** (layout in chờ G10) |
