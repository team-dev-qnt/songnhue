...
| **Phase 3** — C3 (GIS/dashboard/báo cáo) + D (HRM) | | 🟨 **Vào được — cửa vào là D (HRM)** | ⛔⛔ **C3 ⛔ không phải cửa vào**: mã GIS đã dựng (WS-35) nhưng **19/19 điểm đo toạ độ NULL** và **0 `INSERT INTO constructions`** ⇒ bản đồ chạy trên **tập rỗng** (luật 7). G8 ⛔ không chặn *viết* C3, nó chặn **nghiệm thu** C3. ⭐ **D (HRM) vào được ngay**: `backend/hr/` mới có 6 `package-info.java`, mà mọi tiên quyết đã có (`org_units` · `CryptoService` AES-256-GCM · Workflow engine · Attachment). **G6** chặn **đúng một** báo cáo `BCNS-07`, ⛔ không chặn 8 chức năng CN-04 còn lại. ⭐ Phần C3 làm được ngay: **wall mode** (thiết bị chốt B8) + **báo cáo MOD-02** (layout in chờ G10). Số đo đầy đủ: `master-tracking.md` §*Đánh giá sẵn sàng vào Phase 3* |
