...
| Endpoint mực nước | `http://songnhue.bhh40.net/api/getmucnuoc.aspx?key=<MA_SO>;` — ⚠ **bắt buộc có dấu `;` ở cuối**, thiếu là trả `not.working`. ⛔⛔ **ĐỔI 26/09/2026 (WS-87)** từ `getmn.aspx`: đường cũ nay trả **200 kèm `not.working`** — nó *trả lời*, và trả lời bằng đúng chuỗi mà hệ đọc thành *"mã số sai"*, nên gọi nhầm đường sẽ gửi người vận hành đi đổi mã số một cách vô ích. ⚠ Đổi hằng số này **⛔ đứng một mình được**: `DiaChiNguon` cắt trùng theo **ĐOẠN**, nên hàng `base_url` nhúng sẵn `/api/getmn.aspx` (vốn đang được cứu) sẽ nối ra `/api/getmn.aspx/api/getmucnuoc.aspx` ⇒ **404** — phép cắt `base_url` ở `V202609261099` là **bắt buộc đi kèm** |
...
| Endpoint lượng mưa | ✅⭐ **ĐÃ CÓ từ 26/09/2026 (WS-87)** — `http://songnhue.bhh40.net/api/getluongmua.aspx?key=<MA_SO>;`, **cùng một mã số** với mực nước. Đo trên nguồn thật cùng ngày: **200**, **15 bản ghi**, định dạng dòng **y hệt** mực nước, giá trị **THẬP PHÂN** (`value=0.0`), đơn vị **mm**, nhịp **1 giờ** (lúc 10:41 mốc vẫn là `10:00`). ⛔⛔ **15 mã ấy RỜI HẲN** 19 điểm đo đang khai ⇒ số đo vào `hydro_unmapped_readings` **nguyên văn + đơn vị nguồn**, và cột lượng mưa vẫn hiển thị `-` cho tới khi Công ty cấp bảng ánh xạ mã ↔ trạm (**G8**, ⛔ còn G3-a). ⛔ Cấm suy điểm đo từ giá trị đo — bản suy đoán trước đó sai **1/4** mã. ⚠⚠ **Quy ước KHUNG GIỜ chưa chốt**: QuanTran xác nhận *"tích luỹ theo giờ, bucket 08:00–08:59"*, nhưng phép đo cho mốc `10:00` **lúc 10:41** ⇒ mốc có thể đi TRƯỚC bucket một giờ; hôm đo mọi giá trị đều `0.0` nên ba cách đọc ⛔ phân biệt được (luật 9). Mốc được giữ **nguyên văn** ⇒ quyết định lấy lại được; phép đo phân xử ghi ở javadoc `Bhh40LuongMuaAdapter`, phải xong **trong mùa mưa** (hạn 31/10) |
