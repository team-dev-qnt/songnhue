...
| NFR-02a | Trang chủ < 3s | — | — | CHƯA ĐO | Công cụ đo nay CÓ (`T63.22`): `frontend/public-web/playwright.hieu-nang.config.ts` — LCP bằng Chromium thật, đo **cả lượt ISR nguội**. ⛔⛔ **Câu cũ ở đây tự mâu thuẫn với luật của chính bảng này** (sửa 25/09, `T85.18`): nó đòi *"một lượt chạy từ máy ở VN"*, mà *máy ở VN* là một **chỗ đứng**, ⛔ phải một **nguồn hợp lệ** — `máy dev` bị bộ canh từ chối đích danh. Hai đường đo được: **(a)** `CI <run-id>` — đưa hồ sơ hiệu năng vào một workflow (runner GitHub ⛔ ở VN nên số đo là **cận trên**, và phải ghi rõ thế); **(b)** `nguồn ngoài` — một dịch vụ đo công khai, nhưng nó gửi URL production ra ngoài nên **chờ QuanTran quyết**. ⛔ Ghi số đo từ máy dev vào đây kể cả khi con số đẹp: nó phụ thuộc mạng, CPU và tiện ích trình duyệt của đúng một cái máy |
