...
| `npm install <pkg>@<ver>` để bump một phụ thuộc **gián tiếp** | Nó **thêm gói ấy vào `dependencies` của `package.json`** — đổi hợp đồng phụ thuộc của dự án qua một lệnh gõ vội. Dependabot chỉ đụng lockfile. Dùng `--package-lock-only`, rồi **đo `git diff package.json`** phải bằng 0 (T42.31) |
| Tin `npm install` nói nó đã đổi gì | Nó ⛔ không nói. **Diff hai bản lockfile** và đếm *đổi / thêm / bớt* — một bump `next` đúng phải là ~18 gói cùng họ, **0 thêm 0 bớt**; con số khác nghĩa là có thứ khác đi kèm (T42.31) |
| Bump một phụ thuộc **được ghim trong bảng vòng đời** | `deploy/vong-doi-phien-ban.tsv` là **nửa thứ hai của cặp**: sửa `package.json` mà quên bảng thì bảng nói sai thứ đang chạy. `VongDoiPhienBanTest` bắt được, nhưng chỉ ở bước 10/10 — chạy nó **ngay sau khi bump** để ⛔ khỏi mất một vòng CI 20 phút (T42.31) |
| `npm audit` còn lỗ hổng sau khi vừa bump | Hỏi **nhánh có đứng trên `dev` mới nhất không** trước khi đi tìm cách vá: một bản vá vừa gộp vào `dev` cùng ngày là lý do phổ biến hơn hẳn. `git rev-list --count HEAD..origin/dev` (T42.33) |
| `git diff dev <nhánh>` để hỏi *"nhánh này gộp chưa"* | Sai phép đo: kho gộp bằng **squash** nên commit ⛔ không truy ngược được, và diff hai chiều đếm cả thứ `dev` thêm vào **sau** lượt gộp — nhánh đã gộp vẫn cho ra hàng trăm tệp khác. Hỏi **PR nào đã merged** rồi khớp `headRefName` (T42.32) |
| Fixture dùng mã **không tra ra bản ghi nào** | Nó chứng minh đường ống, ⛔ không chứng minh **chặng cuối**. `MockAdapter` trả mã `Z9000x` ⇒ lượt polling khép vòng vẫn xanh với `written_count = 0`, và phép quy đổi đơn vị + ghi bảng dẫn xuất ⛔ chưa ai đi qua. Trộn **mã có thật + mã lạ** trong cùng một mẻ (T42.29) |
| Bài kiểm ghi vào bảng **append-only** | ⛔ Không dọn được, và một `DELETE` ở đó ⛔ không phải dọn dẹp — nó phá chính bảo đảm ấy. Hệ quả dây chuyền: bản ghi con giữ khoá ngoại nên bản ghi cha cũng ⛔ không xoá được ⇒ chuyển trạng thái (`TAM_DUNG`) thay vì xoá (T42.29) |
| Nguồn ngoài cần **máy chủ giả** trong bài kiểm | Đếm **số lượt gọi** máy chủ ấy. Bộ chặn SSRF từ chối `127.0.0.1` theo mặc định, và khi nó từ chối thì mọi khẳng định khác vẫn có thể xanh — vì chúng nói về một **nhánh thoát khác**, ⛔ không byte nào rời máy (T42.29) |
