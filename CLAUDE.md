...
**Codebase đo ngày 3/9 (sau PR #83 — chuông CVE, vòng đời phiên bản, 5 nợ CI/CD)**: **861 test BE** (249 core + 41 content + 41 operations + 14 hydro + **516** app — `app` 480→516 khớp đúng 24 bài của `b0c914e` + 12 bài của `356cf74`) + **453 test FE** (187 admin + 266 public) + **30 phép đo Playwright** (chưa vào CI — T38.10) · **79 mã lỗi** (BE = FE = properties — +HYD-1002/2005/2006 của WS-28; bài canh đếm còn kẹt ở 76 nên CI sẽ đỏ, đã vá) · 88 quyền / 12 vai trò / 334 dòng phân quyền · **36 bài ArchUnit** (7 lớp, gồm **14 bài tự-kiểm** — +3 bài chứng minh ngoại lệ "cột phạm vi NULLable" của `Station` không bị lạm dụng) · **11 phép kiểm bộ đọc tracking** · **bộ canh thứ tự migration** (script so nhánh nền) **+ `MigrationNamingTest` 5 bài** canh dãy `nnnn` tăng dần — bắt được ngay 2 tệp lịch sử `V202608241255/1256` đánh số bằng giờ-phút · 47 migration, đỉnh `V202609011051` · mọi cổng bao phủ **chạy thật** · CVE ≥ 7: **0** (đo 28/8, `tomcat.version` 10.1.59 — số đo có hạn dùng, **chưa đo lại 1/9**) · **0 mã màu ghi cứng** trong `public-web` (admin-app còn 25 — nợ T25.23). ⛔ Mọi con số trên là **số đo có hạn dùng**: ghi vào sổ thì phải ghi kèm ngày đo.
