...
- ➡️ **Bước tiếp theo**: gửi 6 mục còn mở + `report-templates-proposal.md` cho Công ty; làm tiếp **WS-6 (Core services — 25 pd, khối lớn nhất Phase 0)** theo `phase0-tracking.md` — trong đó **ArchUnit và khung giám sát dữ liệu quá hạn phải nằm trong Phase 0**, load test 200 CCU đưa vào kế hoạch từ Phase 2. WS-6 nhận **5 dòng nợ**: T6.3 (MinIO bean) · T6.4 (ClamAV/EXIF) · T6.8 (`TokenMaintenanceJob` + job tạo partition audit) · T6.11 (Settings phần ghi) · T6.15 (`AuthorityLoader.invalidate`).
