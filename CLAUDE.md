...
- ➡️ **Bước tiếp theo**: gửi 6 mục còn mở + `report-templates-proposal.md` cho Công ty; làm tiếp **WS-7 (Backup/Restore & Observability — 9 pd)** hoặc **WS-10 (Test & CI — 10 pd)** theo `phase0-tracking.md`. WS-10 đang giữ **4 dòng nợ nặng nhất**, trong đó 2 mục hỏng âm thầm: luật ArchUnit bắt lớp con `ScopedEntity` mang `@Filter` và bắt `WorkflowAware.applyState` chỉ gọi được từ `WorkflowEngine`.
