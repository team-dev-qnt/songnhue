...
⚠⚠ **Hai mươi bốn lượt liên tiếp một bản ghi "đã xong" bị lượt rà sau bác bỏ.** Đây là hình dạng rủi ro
...
**Codebase đo ngày 29/8 (sau lượt rà CI/CD §10.68)**: **720 test BE** (249 core + 37 content + 33 operations + 401 app) + **300 test FE** (142 admin + 158 public) · **74 mã lỗi** (BE = FE) · 88 quyền / 12 vai trò / 334 dòng phân quyền · **32 bài ArchUnit** (7 lớp, gồm 11 bài tự-kiểm chứng minh luật bắt được vi phạm) · **11 phép kiểm bộ đọc tracking** (nay CÓ trong `ci.yml` + `make ci-local` bước 1/10, gồm 2 bài canh **tiến trình MCP chạy đúng mã trong kho**) · **bộ canh thứ tự migration** (bước 2/10 + job CI `Thứ tự migration`) · mọi cổng bao phủ **chạy thật** (content 18.2%) · **CVE ≥ 7: 0 tính tới 27/8, rồi 9 mã ở `tomcat-embed-core` 10.1.57 nổ ra ngày 28/8 mà không ai đụng vào mã** — vá bằng `tomcat.version` 10.1.59 và **đã nghiệm thu bằng lượt quét thật** (run 33253652221: **110 artifact**, báo cáo mang đúng `tomcat-embed-core-10.1.59.jar`, **0 mã ≥ 7**) (§10.68). ⛔ Con số này là **số đo có hạn dùng**, không phải thuộc tính của dự án: ghi nó vào sổ thì phải ghi kèm ngày đo · **0 mã màu ghi cứng** trong `public-web`; **ép ALL CAPS chỉ còn ở `PortalNav`** (3 chỗ, Công ty yêu cầu 28/8 — bộ canh thu hẹp kèm ngưỡng đếm, mọi tệp khác vẫn cấm) (admin-app còn 25 mã màu — nợ T25.23; chữ hoa ở admin-app chưa rà).
