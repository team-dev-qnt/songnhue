...
⭐ **Phase 3 vào được — cửa vào là D (HRM), ⛔ không phải C3** (đo 8/9, chi tiết ở `master-tracking.md` §*Đánh giá sẵn sàng vào Phase 3*): `backend/hr/` mới có **6 tệp, cả 6 là `package-info.java`**, mà mọi tiên quyết đã có (`org_units` dùng chung · `CryptoService` AES-256-GCM cho `employee_sensitive` · Workflow engine cho nghỉ phép · Attachment service). **G6** chặn **đúng một** báo cáo `BCNS-07`, ⛔ không chặn 8 chức năng CN-04 còn lại.
...
⛔ **Ràng buộc thứ tự**: `T11.69` Boot 4.1.1 hạn cứng **15/10** phải xong **TRƯỚC** khi bấm giờ `T37.1` — 7 ngày quan sát phải chạy trên **đúng stack sẽ lên production**.
...
⭐ **Đo lại 09/09/2026 trên nhánh `fix/g6-g8-g10`, `make ci-local` thoát 0 — cả 10/10 bước**: **1611 testcase BE** · **0 đỏ** · **208 báo cáo surefire** · FE **345** test admin-app / 43 tệp + **352** test public-web / 39 tệp · **67 migration**, đỉnh `V202609091073__hyd_g8_tuyen_song_ly_trinh` · **887 dòng** sổ tracking đọc được, 0 phép kiểm đỏ · mã lỗi **102** (+`OPS-2022`; cổng đếm hai phía bắt được nó ở lượt chạy đầu). ⚠ Vẫn là số **ở MÁY** — hai job chỉ sống trên runner (quét CVE · đóng gói image) ⛔ không dựng lại được ở đây.
...
- **G8**: bản chụp của Công ty đã vào CSDL (`V202609091073`) — **tuyến sông 13/19 · lý trình 10/19**;
  ...
  GIS điểm đo vẫn RỖNG** và G8 vẫn chặn nghiệm thu C3. ⚠ `F01519` Lương Cổ đổi **TL → HL** theo bản
  chụp (hai nguồn của Công ty lệch đúng 1/19 dòng — đã đưa vào thư hỏi lại).
- **G8 đường nhập**: 4 khuyết tật đã vá — trần 5.000 dòng **cắt cụt im lặng** → ném `OPS-2022` ·
