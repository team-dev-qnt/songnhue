...
⭐⭐ **19/09 (WS-69) — sao lưu khôi phục được thật** (`T37.8` ✅ · `T68.4` ✅ · `T68.3` 🟨): bản dump đêm giữ ACL; nút M5.11 đi cùng khuôn script container và đọc CÙNG khối SQL (`deploy/backup/truoc-khi-nap.sql`); `ADM-2009` có nơi ném. ⛔⛔ Đo ra **lỗi thứ tư** mà cả script đã đo 08/09 cũng mang: khôi phục ĐÈ lên CSDL đã migrate cho `songnhue_app` **72 quyền thừa** trên đúng các bảng append-only — cơ chế là quyền MẶC ĐỊNH của đích, ⛔ phải `--no-privileges` như §10.80 ghi (`architecture-review.md` §12.3). ⛔ Staging có thể VẪN cho app sửa/xoá nhật ký kiểm toán từ 26/08 ⇒ **`T69.6`** (QuanTran đo cả hai máy, lệnh ở `phase4-tracking-tmp.md` §B4). Số ở MÁY, `make ci-local` thoát 0: **2072** testcase BE (core 362 · content 55 · hydro 229 · operations 80 · app 1346) · 0 đỏ · FE 558/88 + 399/46 · 82 migration ⛔ thêm.
