...
| `docs/cicd.md` | Luồng CI/CD, cổng đề bạt, secret cần đặt. ⭐ **Đơn giản hoá 18/09** (T66.9): test **chỉ ở PR** vào `dev` · image **chỉ ở push `dev`** · PR `dev → staging` và `dev → production` **độc lập** (⛔ còn bắt đi qua staging), **0 người duyệt**, guard = nguồn `dev` + CI xanh + gốc chung · vẫn merge commit khi đề bạt |
...
⭐⭐ **19/09 (WS-69) — sao lưu khôi phục được thật** (`T37.8` ✅ · `T68.4` ✅ · `T68.3` 🟨): bản dump đêm giữ ACL; nút M5.11 đi cùng khuôn script container và đọc CÙNG khối SQL (`deploy/backup/truoc-khi-nap.sql`); `ADM-2009` có nơi ném. ⛔⛔ Đo ra **lỗi thứ tư** mà cả script đã đo 08/09 cũng mang: khôi phục ĐÈ lên CSDL đã migrate cho `songnhue_app` **72 quyền thừa** trên đúng các bảng append-only — cơ chế là quyền MẶC ĐỊNH của đích, ⛔ phải `--no-privileges` như §10.80 ghi (`architecture-review.md` §12.11). ⛔ Staging có thể VẪN cho app sửa/xoá nhật ký kiểm toán từ 26/08 ⇒ **`T69.6`** (QuanTran đo cả hai máy, lệnh ở `phase4-tracking-tmp.md` §B4). Số ở MÁY, `make ci-local` thoát 0: **2072** testcase BE (core 362 · content 55 · hydro 229 · operations 80 · app 1346) · 0 đỏ · FE 558/88 + 399/46 · 82 migration ⛔ thêm.
⭐⭐ **19/09 (WS-70) — TLS · cổng · khoá, phần trong kho** (`T60.10` ✅ · `T68.13` ✅ · `T11.88` · `T11.54` · `T68.37` 🟨): `host-prepare.sh` cài `cron` + ĐÚNG dòng gia hạn TLS và `--kiem` kiểm kê cổng đang nghe; chuông mới `canh-chung-chi.yml` đo hạn 7 tên miền từ runner mỗi sáng; admin luôn `noindex` qua khoá REGEX của `map` (đo: khoá chính xác 99 byte ⇒ `[emerg]`, regex ⇒ đạt); đồ gá token giả dựng lúc chạy + bộ canh hình dạng bí mật toàn kho. ⛔⛔ Nhãn `canh-cong-quet` ⛔ TỒN TẠI ⇒ nhánh mở issue của chuông T11.67 chưa từng chạy (T70.3). ⬜ **`T70.7`**: VPS-2 chạy `host-prepare.sh` trước **23/10**. Số ở MÁY, `make ci-local` thoát 0: **2092** testcase BE (app 1366) · FE 558 + 399.
⭐⭐ **19/09 (WS-71) — quay lui trả CẢ cấu hình** (`T11.95` ✅ · `T11.9` 🟨 · DOD0.21 đủ điều kiện đo): CD chụp `/opt/songnhue` vào `.ban-truoc/` trước khi đồng bộ; hỏng thì trả bản chụp của ĐÚNG lượt → `nginx -t` → `--force-recreate` → so ID ảnh → chờ nginx healthy (ngày 17/09 quay lui chỉ-ảnh ⛔ cứu được lỗi cấu hình nginx); quay lui tay dùng `deploy/` của chính SHA; thử khô rsync DỪNG nếu `--delete` sắp xoá tệp crontab/systemd đang gọi; bản sao `.env` về `~/.songnhue-env-bak/`. ⬜ **`T71.6`**: diễn tập DOD0.21 bằng biến `DIEN_TAP_QUAY_LUI` ở environment staging. Số ở MÁY, `make ci-local` thoát 0: **2104** testcase BE (app 1378) · FE 558 + 399.
⭐⭐ **19/09 (WS-72) — hạn mức sau NAT, phần làm được mà ⛔ chờ đo** (`T61.6` ✅ · `T61.17` 🟨 · `T61.29` 🟨): xô LOGIN theo IP chỉ giữ lượt ⛔ đúng mật khẩu — bộ lọc vẫn tính TRƯỚC (nguyên tử), mật khẩu đúng thì trả lại đúng lượt ấy (`architecture-review.md` §12.12: ⛔ "chỉ đọc rồi đếm lượt sai" vì lượt đồng thời cùng lọt; ⛔ nối `reset` vì một tài khoản thật xoá được xô); nginx trả **429** thay 503; trang tìm kiếm có trạng thái *"Chưa tra cứu được"* thay vì *"Không tìm thấy"* khi backend chưa trả lời, và bộ tải thử đếm nó; Playwright quản trị đăng nhập MỘT lần mỗi dự án. ⚠ Lượt CI đầu của #175 đỏ vì GNU `stat -f` là *hệ tệp* — biến thể thứ tư của *"xanh ở máy ⛔ phải bằng chứng"* (macOS/BSD ≠ runner Linux/GNU, `T71.8`). ⬜ `T61.17`: nginx `api_auth` chờ phép đo NAT. Số ở MÁY, `make ci-local` thoát 0: **2116** testcase BE (core 368 · app 1384) · FE 558 + 405.
⭐⭐ **19/09 (WS-73) — bảo mật còn lại, đợt 1** (ASVS 4.2.1 · 14.4.2 ✅ · `T68.16` ✅ · `T68.40` 🟨): 6 lượt tra entity phạm vi theo `publicId` nay qua `ScopeGuard` (đơn vị khác ⇒ 403 `AUTH-3002` + dấu vết, ⛔ 404 im lặng) — sổ ghi 5, đo toàn kho ra 6 — và luật bytecode neo vào KIỂU TRẢ VỀ giữ từ nay; banner thôi nhận `javascript:` (đường ghi thứ BA ra cổng); JSON của API mang `filename="api.json"`; bộ canh *mọi mã ASVS §16 có dòng sổ giữ* ở job `tracking` (đo ra 24/57 mã mồ côi); chỉ `EmailSender`/`MailConfig` chạm API gửi thư. ⛔⛔ Lượt `verify` đầy đủ lộ ra **mọi luật ArchUnit soi cả 82 lớp KIỂM của `core`** — test-jar lọt qua `DoNotIncludeTests`, chỉ lượt chạy toàn bộ thấy (T73.10). ⚠ Một khẳng định của chính tôi (*"đặt header ở filter sẽ bị nhân đôi"*) bị lượt phá bác: Spring 7.0.9 THAY header. ⬜ WS-73b: 2.3.1 · 11.1.2; `T73.7` CSP cổng. Số ở MÁY, `make ci-local` + `make ci-order` thoát 0: **2128** testcase BE (app 1396) · FE 558 + 406.
⭐⭐ **20/09 (WS-73b) — mật khẩu tạm có hạn · vé biểu mẫu công khai** (ASVS 2.3.1 vế hết hạn ✅ · 11.1.2 ✅ · `T68.40` 🟨): mật khẩu tạm hết hiệu lực sau `security.password.temp-ttl-hours` (72 giờ) ⇒ `AUTH-0010`, chặn SAU khi mật khẩu đúng; cột riêng `users.temp_password_expires_at` (⛔ suy từ `password_changed_at` — suy ra là khoá ngược mọi mật khẩu tạm đang lưu hành) · biểu mẫu Liên hệ/Góp ý đòi **vé do máy chủ ký** đủ `security.form.min-fill-seconds` (3 giây) và ≤ 24 giờ ⇒ `CMS-2025`; cổng xin vé ở `onFocus` và **tự chờ** đủ tuổi, ⛔ trả lỗi cho người điền nhanh. Lý do từng lựa chọn: `architecture-review.md` §12.13. ⬜ Vế *hệ sinh mật khẩu tạm* ⇒ `T73.11` chờ QuanTran chọn. ⚠ Lượt `ci-local` thứ hai **⛔ hề chạy**: `rm -rf backend/*/target/…` gặp glob rỗng, zsh báo `no matches found` và chuỗi `&&` dừng — log trên đĩa là log CŨ, chỉ lộ ra nhờ đọc mốc giờ trong log (luật 32). Số ở MÁY, `make ci-local` + `make ci-order` thoát 0: **2146** testcase BE (core 377 · app 1405) · FE 561 + 419 · **84 migration** · mã lỗi **138**.
⭐⭐ **20/09 (WS-74) — nghỉ phép đúng luật, thư duyệt đúng người** (`T68.10` ✅ · `T57.15` ✅ · `T57.16` 🟨): phép năm = cơ sở + 1 ngày mỗi **đủ** 5 năm (Điều 113 + **114** — bản 3 bậc 12/13/14 thiếu 1–2 ngày cho người ≥ 10 năm; migration giữ cơ sở đang dùng và **dừng** nếu hai bậc trên bị sửa tay) · *"biết năm trước"* là tham số `hr.leave.first-fully-recorded-year` (2027), ⛔ đoán từ *"có ≥ 1 đơn"* — phép đoán ấy cấp thừa cho năm dở dang 2026 và xoá số chuyển của người ⛔ nghỉ ngày nào · thư đơn mới / chuyển cấp 2 / rút đơn chỉ tới người có quyền **mà phạm vi phủ đơn vị người nộp** — bài kiểm là một tương ứng *nhận thư ⇔ thấy đơn*. Bộ canh mới `HrSettingsReadTest` (nhóm `hr.*` trước nay ⛔ ai giữ). ⚠ Hai phát hiện: javadoc trỏ tới `ChinhSachPhepHttpTest` **⛔ tồn tại** (T74.4) · thư chỉ-báo-chủ-bản-ghi **cộng Ban điều hành** — đo được 1 thư/đơn nghỉ được duyệt khi nhóm ấy có người (T74.7 → WS-75). Số ở MÁY, `ci-local` + `ci-order` thoát 0: **2157** testcase BE (app 1416) · FE 561 + 419 · **86 migration**. ⚠ Lượt CI đầu đỏ 3 bài vì mẫu dọn của chính bài kiểm mới (xem bảng trên, 20/9).
⭐⭐ **20/09 (WS-74b) — vế GHI của phạm vi đơn vị** (ASVS 4.2.1 vế ghi ✅ · `T73.1` chỉ phủ vế đọc): đo trước khi viết bộ nhập CBNV ⇒ tài khoản ở XN-A **tạo/chuyển** hồ sơ CBNV · công trình · điểm đo sang XN-B đều **201**, ⛔ dòng nhật ký nào — quyền đi theo vai trò còn phạm vi đi theo đơn vị của tài khoản, và `TECHNICIAN` (thường đặt ở Xí nghiệp) giữ đúng hai quyền ghi ấy. ⇒ `ScopeGuard.requireWritableOrgUnit` (cùng vị từ, cùng nguồn phạm vi với bộ lọc đọc) + luật bytecode **W1** ĐO mọi chỗ đặt đơn vị ở tầng application và bắt **xếp loại** (3 dòng miễn: đơn vị SAO từ bản ghi đã qua `ScopeGuard`). Kèm `toanCongTy` cho 7 phép kiểm trùng mã — trước đó mã của đơn vị khác VÔ HÌNH nên người dùng nhận `SYS-0005` *"dữ liệu vừa được người khác thay đổi"* (luật **W2** canh). ⬜ `T74.11`: ô chọn đơn vị vẫn bày cả cây. ✅ **đóng 25/09 (WS-85)** — cờ `chiTrongPhamVi` + đường `/org-units/selectable`; đệm dùng chung khoá an toàn được là nhờ `T85.13` xoá **toàn bộ** `queryClient` ở `endSession()`, gồm cả đường token hết hạn. Số ở MÁY, `ci-local` + `ci-order` thoát 0: **2166** testcase BE (app 1425) · FE 561 + 419 · ⛔ migration mới.
⭐⭐ **20/09 (WS-74c) — bộ nhập danh sách CBNV + ô ngày của Excel** (`T68.23` ✅ — G6-a nay *gửi danh sách là nhập được ngay*): `EmployeeImportService` + tệp mẫu **16 cột do backend sinh** (tệp tĩnh `docs/mau/mau-danh-sach-cbnv.csv` đã XOÁ — nó kịp lệch lược đồ ở *Thời vụ* và *Sau đại học*) + nút *Nhập từ tệp*; nhập đi **qua `EmployeeService`** nên thừa hưởng chuẩn hoá mã, ràng buộc trạng thái ↔ ngày nghỉ, phạm vi ghi (WS-74b) và audit. ⛔⛔ **Ô TRỐNG là GIỮ NGUYÊN**, ⛔ phải lệnh xoá — hồ sơ 24 trường / tệp 16 cột, và bài phản chiếu đòi bộ hợp nhất chép ĐỦ 24 thành phần. ⭐ `SpreadsheetReader` nay đọc được **ô ngày của `.xlsx`** (số sê-ri + `styles.xml` + cờ `date1904` + lỗi năm nhuận Lotus) — javadoc của chính nó đã dặn từ WS-17 rằng *cột ngày đầu tiên xuất hiện thì phải xử lý Ở ĐÂY*, và đây là cột ngày đầu tiên. Số ở MÁY, `ci-local` + `ci-order` thoát 0: **2176** testcase BE (core 380 · app 1432) · FE 564 + 419 · ⛔ migration mới.
⭐⭐ **20/09 (WS-82) — thư đi đúng người: tách *báo cho chủ* khỏi *cảnh báo G11*** (`T74.7` ✅ · `T76.3` ✅ · `T68.18` ✅): `RecipientResolver` thôi **SUY** chính sách người nhận từ `targetPermission == null` — cờ `nhomCanhBao` nay do nơi gọi **KHAI RA** trên `NotifyRequest`/`NotificationRequest`, và chỉ `alert(...)` + `AlertNotifier` khai `true`. ⛔⛔ Phạm vi đo lại **rộng gấp ba dòng nợ mô tả**: ⛔ chỉ `WorkflowEngine` (**17** hàng `notify_owner` ∧ `notify_permission IS NULL`) mà còn `UserAdminService` + `CanhBaoTaiKhoanService` ⇒ **6 mã sự kiện an ninh của MỘT cá nhân** (*"tài khoản của bạn đã bị khoá"*) đang cộng cả ban lãnh đạo. ⛔⛔ **Hai dòng nợ CHE NHAU**: nhóm seed `'[]'` và chưa ai điền được (ô JSON thô đòi `publicId` mà màn hình Tài khoản ⛔ hiện) ⇒ lỗi **đang ngủ**; trả `T76.3` một mình là **bật nó** trên cả 6 mã sự kiện + 17 hàng cùng lúc ⇒ phải vá **cùng một PR**, `T74.7` trước. ⛔ Và **cách vá dòng nợ tự kê ra cũng sai** (cột trên `workflow_transitions`): chạm 1/3 nơi gọi, ⛔ hàng seed nào cần `TRUE` ⇒ một **cột chết** (luật 15). ⚠ Vế phân biệt bản ĐẦU của tôi là **xanh giả** — dữ liệu `'{"a":1}'` rơi sang nhánh *giá trị hỏng* chứ ⛔ nhánh *khoá nào*, nên bản phá vẫn 4/4 xanh; đổi sang mảng hợp lệ ⇒ đỏ đúng 1 bài. Lý do từng lựa chọn: `architecture-review.md` **§12.19**. Số ở MÁY, `ci-local` + `ci-order` thoát 0 **cùng một bộ số**: BE **2219** (core 395 · content 55 · hydro 229 · operations 81 · app 1459) · 0 đỏ · FE **645**/99 + **457**/52 · **91 migration** ⛔ thêm · mã lỗi **146** ⛔ đổi.
⭐⭐ **20/09 (WS-82 đợt 2) — ba dòng nợ nặng nhất còn lại** (`T68.31` ✅ · `DOD1.16` ✅ · `T73.7` ✅ · `T80.7` 🟨): mỗi cái một **cơ chế sống sót** khác nhau (`architecture-review.md` §12.20). ⛔⛔ **`T68.31`** — `hr/src/test` có **0** tệp ⇒ JaCoCo in `Skipping … missing execution data file` rồi build **XANH**: cổng bao phủ của cả module vắng mặt mà ⛔ gì báo (luật 7 + luật 31). Và sàn cha **0.18** là số đo Phase 0 trong khi đo 20/09 ra core **0.3490** · content **0.3873** · hydro **0.5490** · operations **0.8028** ⇒ bánh cóc **thôi cóc**. Nay mỗi module khai ngưỡng RIÊNG bằng đúng số đo, `operations` 0.70 → **0.80**, `hr` có **19 testcase** chỉ nhắm 5 chỗ mang luật thật (⛔ đệm tỷ lệ — chính pom cha dặn thế), và `BaoPhuDomainCoChayTest` **ĐO** danh sách module từ đĩa. Phép đo quyết định: ngưỡng `hr = 0.99` ⇒ `Rule violated for bundle songnhue-hr: 0.25` — cổng **sống** ở đúng chỗ trước im lặng. ⛔⛔ **`T73.7`** — lý do giữ `'unsafe-inline'` **đúng vào ngày viết** rồi hết đúng: `next build` cho **18/19 route ĐÃ là `ƒ`**, bảng ⛔ có dòng `●` nào ⇒ giá của nonce **= 0 route** (thêm middleware xong bảng route **y hệt**). Chỗ lập luận cũ sai là **trộn hai cơ chế cùng tên "ISR"**: NFR-02 dựa vào bộ đệm **dữ liệu fetch**, ⛔ phải bộ đệm **trang**. Đo trên MÁY CHỦ THẬT: `script-src 'self' 'nonce-…'`, **11/11** thẻ `<script>` mang đúng nonce, nonce khác nhau mỗi request, `/api/health` vẫn 200. ⚠ **Một khẳng định của chính tôi bị đo bác**: *"thiếu header CSP của request ⇒ trắng trang"* — sai, Next 16.3.5 nhận **cả hai** header, mỗi cái một mình đã đủ. ⛔⛔ **`T80.7`** — dòng nợ **kê sẵn một cách vá BẤT KHẢ** (*"ca thứ năm của `RecipientResolver`"*): `core` ⛔ import được `hr` (quy tắc 6) nên nó ⛔ thấy `UyQuyenDuyetPhep` ⇒ sẽ **bỏ sót đúng người được uỷ quyền**. Lần thứ **hai trong hai ngày** một dòng nợ kê sai cả bản vá (§12.19 · §12.20) ⇒ phần *"⬜ DEV:"* cũng là **dữ liệu chưa kiểm**. Thứ bắt được là **bài kiểm CÓ SẴN**: `expected ["A (XN-A)", "C (gốc)"] but was ["A (XN-A)"]` — C thấy đơn, nhận thư, mà ⛔ có nút. ⬜ Ba sự kiện do `WorkflowEngine` phát vẫn theo phạm vi ⇒ `T82.5`. Số ở MÁY, `ci-local` + `ci-order` thoát 0 **cùng một bộ số**: BE **2243** (core 395 · content 55 · hydro 229 · operations 81 · **hr 19** · app 1464) — đo lại SAU khi rebase lên `dev` `4629dd8e`; phép cộng khép kín `2217` (đỉnh `dev` sau #190) `+ 3` (`NhomCanhBaoKhongLanSangThuCaNhanTest`) `+ 4` (`BaoPhuDomainCoChayTest`) `+ 19` (`hr`, lần đầu có báo cáo) · 0 đỏ · FE **645**/99 + **460**/53 · **91 migration** ⛔ thêm · mã lỗi **146** ⛔ đổi.
⭐⭐ **25–26/09 (WS-85) — trả nốt nợ PHẦN MÃ: 34 commit trong MỘT PR (#208), mỗi chủ đề một commit qua trọn `make ci-local` ở máy** (+ `ci-order` khi chạm backend). ID và số đo từng dòng ở sổ; ba mục **⛔ đóng được bằng mã** đã ghi lý do ĐO ĐƯỢC thay vì tick: `T37.3`+`DOD1.17` (`T85.18` — *"máy ở VN"* là một **chỗ đứng**, ⛔ phải một nguồn đo hợp lệ; `make ci-local` là cổng để ⛔ đẩy mã hỏng, ⛔ phải phép đo nghiệm thu) · `T40.13` (chặn bởi `T38.10`) · `T85.17` (chờ Công ty). ⛔⛔ **Việc CHẶN của đợt là HẠ TẦNG, ⛔ phải mã**: `compose.*` ghim ảnh MinIO mà **cả Docker Hub lẫn quay.io đã gỡ** `minio/minio` trong vòng mười ngày ⇒ nhân bản sang GHCR của chính dự án (`T85.19`), và ảnh ấy nay **tồn tại ở đúng một nơi trên đời** ⇒ bản sao ngoại tuyến là việc ⛔ hoãn được. ⭐⭐ **Phép đo đóng được đợt này là phép so TỪNG MODULE giữa runner và máy**, ⛔ phải màu của lượt CI: một lượt xanh mà `app` chỉ chạy 620 bài in ra **màu y hệt** lượt chạy đủ 1558.
...
⬜ **Những gì còn lại của Phase 3 ⛔ KHÔNG phải mã** — cả sáu đều chờ Công ty hoặc chờ một quyết
...
trong khi Công ty có ca trực.
⚠⚠ **Sửa 26/09 — dòng này từng xếp `T59.13/14` vào nhóm *"⛔ phải mã, chờ Công ty"*, và đó là XẾP NHẦM**: xuất ảnh bản đồ và bộ đọc KML/KMZ là **mã thuần**, ⛔ chờ ai; đã trả ở WS-85 (`xuatAnhBanDo.ts` **0 phụ thuộc mới** · `DocKmlSangGeoJson.java`). ⛔⛔ Một mục xếp nhầm vào ô *"chờ
người khác"* thì **⛔ ai đi làm nó** — và hướng sai này nguy hiểm hơn ghi thiếu, vì mục ấy **CÓ** người canh, người ấy chỉ đang canh nhầm cửa.
...
| 20/9 | ⛔⛔ **Mẫu DỌN của một bài kiểm mới xoá một khoá `settings` THẬT** — `LIKE 'hr.leave.annual-days.%-years'` khớp luôn `…seniority-step-years` ⇒ ba bài phép năm đỏ với *"hệ cho 12, luật 13"*. ⚠ `make ci-local` **và** `make ci-order` đều XANH: ci-order chỉ là *một* thứ tự khác, ⛔ phải thứ tự của runner. Một mẫu dọn cũng là một phép so — nó phải phân biệt được hàng ĐỒ GÁ với hàng THẬT | T74.1 |
...
| 25/9 | ⛔⛔ **Một phép ĐỌC-NGƯỢC báo *khớp: True* trong khi CẢ 33 dòng đều sai — nó chứng minh chỗ LƯU, ⛔ chứng minh nội dung ĐÚNG.** Thân PR sinh bằng `git log --format` đặt trong nháy ĐƠN của shell ⇒ dấu chéo ngược đi vào nguyên văn, mọi dòng commit mang cặp nháy huyền bị thoát. Lượt đọc-ngược so *thứ tôi gửi* với *thứ GitHub trả về* nên nó xanh — **hai vế cùng sai như nhau**. Thứ bắt được là một khẳng định về **HÌNH DẠNG**: đếm số dòng commit phải bằng 33, cộng `assert` ⛔ có ký tự chéo ngược nào. Luật 9 ở hình dạng mới: *lưu đúng thứ đã gửi* và *gửi đúng thứ* là hai câu hỏi khác nhau | T85.19 |
| 26/9 | ⛔⛔ **Hai ngày mô tả ĐÚNG triệu chứng mà chẩn đoán vẫn để ngỏ — thứ đóng nó lại là một NHÓM ĐỐI CHỨNG, ⛔ phải thêm một phép đo trên nạn nhân.** Gói MinIO trên GHCR là `private` và CI trả `denied` 500 ⇒ giả thuyết hiển nhiên: *"vì nó riêng tư"*. Nhưng `songnhue/app`·`admin-app`·`public-web` **cũng `private`** mà CI kéo ngon **mỗi lượt** — khác biệt duy nhất đo được là chúng có trường `repository` ⛔ `null`; tổ chức bật *Inherit access from source repository*, nên một gói `repository=null` **⛔ có gì để kế thừa**. ⇒ `private` **chưa bao giờ** là nguyên nhân; thiếu **liên kết kho** mới là. Nối kho ⇒ `denied` 22 → **0**, reactor `app` FAILURE → **7/7 SUCCESS**. ⚠⚠ Và **cả hai lý do tôi nêu cho hai phương án hôm trước đều SAI** khi đo lại — lần thứ MƯỜI MỘT của *một dòng nợ tự nó sai*, lần này nằm ngay trước mặt người phải quyết | T85.19 |
...
hai** gói tiếp theo — ⛔ có trong `T61.48` (chờ quyết định) lẫn `T61.49` (chờ đo trên máy chủ — ✅ **đóng 25/09 (WS-85)**, `DauVaoMaHoaHieuXacDinhHttpTest`). `endSession()` dọn token,
...
⭐⭐ **Đo lại 26/09/2026 sau WS-85 (trả nốt nợ phần mã — 34 commit, PR #208) — `make ci-local` thoát 0**,
tiến trình maven DUY NHẤT (⚠ số ở **MÁY**): **2360 testcase BE** — core **418** · content 55 · hydro 229 ·
operations 81 · hr 19 · app **1558** · **0 đỏ**, **0** `Rule violated` · FE **741** admin-app / 112 tệp +
**494** public-web / 56 tệp · **92 migration** (89 `db/migration` + 3 `db/seed`; +1 tệp chỉ-test nằm
ngoài đếm), đỉnh `V202609221098__hr_thu_duyet_theo_tham_quyen` · **92 vân tay** · mã lỗi **146** ⛔ đổi
(`T42.28` là lượt **ĐỔI TÊN** `OPS-2016`/`OPS-2015` → `SYS-0015`/`SYS-0016`, ⛔ đúc mã mới) · sổ
**2467 dòng**, 0 phép kiểm đỏ.
⭐⭐ **Con số đáng giá nhất ⛔ phải tổng bài kiểm, mà là sáu con số trên TRÙNG KHÍT giữa runner và máy**
(lượt `36114835013`). Một lượt CI xanh mà `app` chỉ chạy 620 bài in ra **màu y hệt** một lượt chạy đủ
1558 — nên chỉ phép so **từng module** mới phân biệt được *đã chạy* với *bị bỏ qua*.
...
- ⭐ **Cơ chế nhập tệp nay DÙNG CHUNG** (`core/common/importer` + `components/business/ImportModal`):
  ...
  bộ đọc dùng** — ⛔ không tệp tĩnh nào. ✅ Ngoại lệ cuối cùng (`docs/mau/mau-danh-sach-cbnv.csv`) đã
  **xoá 20/09/2026** khi bộ nhập CBNV dựng xong (T68.23) — nó đã kịp lệch lược đồ đúng như dự đoán.
...
| Biến kho `PUBLIC_SITE_URL` | ⭐ **6/9 đã đặt** = `https://songnhue.com` → ⚠ **08/09 đổi sang `https://thuyloisongnhue.vn`** (T11.94, đo lại 14/09). ✅ T11.7-a đã đóng (sitemap production 0 lần `localhost`). ⚠ Nhưng `SITE_URL` vẫn nướng LÚC BUILD và ảnh dùng chung ⇒ robots/sitemap/canonical của **staging** tự nhận là production (đo 19/09) — `T68.12`. ✅ **`T68.12` đóng 25/09 (WS-85)**: `SITE_URL` nay đọc **lúc chạy**; `robots.ts` mở khoá bằng `connection()` chứ **⛔ `force-dynamic`** (thứ bị cấm trên cổng vì nó hạ mặc định đệm của mọi route); cổng canh là `SiteUrlDocLucChayTest` + `robots.test.ts`. ⇒ Dòng `NEXT_PUBLIC_SITE_URL` trong `.env` **cả hai máy** nay THỪA — xoá được, cùng biến kho `PUBLIC_SITE_URL`. |
