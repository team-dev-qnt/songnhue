...
- ➡️ **Bước tiếp theo**: gửi 6 mục còn mở + `report-templates-proposal.md` cho Công ty; làm **WS-8 (FE admin-app — 15 pd)** hoặc **WS-11 (Deploy — 10 pd)**. WS-11 đang giữ nhiều nợ nhất: #26 (VM + compose staging/prod), #29 (VM-3 + Prometheus/Grafana), #30 (Alertmanager), #31 (nginx chặn actuator/swagger).
