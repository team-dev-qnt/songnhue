...
export async function SiteFooter() {
  ...
  // T61.34 — chữ tự do do quản trị nhập ⇒ chỉ thành href khi an toàn (React 18 ⛔ chặn `javascript:`).
  const heThongVanBan = lienKetAnToan(config?.['site.external.doc-system-url']);
  ...
  const social = [
    {
      ...
      url: lienKetAnToan(config?.['site.footer.social.facebook']),
      icon: (
    ...
    },
    {
      ...
      url: lienKetAnToan(config?.['site.footer.social.zalo']),
      icon: (
    ...
    {
      label: 'Kênh YouTube',
      url: lienKetAnToan(config?.['site.footer.social.youtube']),
      icon: (
